...
// 👉 Reemplaza con tus clientes reales (logos opcionales)
...
export default function HomePage() {
    ...
    <main>
      ...
      <section id="inicio" className="section hero-wrap hero-grid">
        ...
        <div className="container-max grid gap-8 lg:grid-cols-[1.25fr_.9fr] lg:gap-12 items-start">
          ...
          <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, ease: 'easeOut' }}>
            ...
            <motion.div className="mt-6 flex flex-wrap gap-3" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.18 }}>
            ...
            {/* Stats */}
            <motion.div
              className="mt-8 grid grid-cols-2 sm:grid-cols-3 gap-5"
              initial="hidden"
              animate="show"
            ...
            >
              {[{ k: '8+ años', v: 'Experiencia' }, { k: '50+', v: 'Proyectos' }, { k: 'End-to-end', v: 'Frontend · Backend · Infra' }].map((s) => (
                <motion.div key={s.k} className="stat" variants={{ hidden: { opacity: 0, y: 8 }, show: { opacity: 1, y: 0, transition: { duration: 0.6 } } }}>
          ...
          >
            ...
            <div className="mt-3 grid grid-cols-2 gap-2">
              {['Next.js', 'Node.js', 'Prisma', 'MySQL', 'TypeScript', 'Tailwind'].map((s, i) => (
                <motion.span key={s} className="pill text-sm" initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.45, delay: 0.25 + i * 0.05 }}>
                  {s}
                </motion.span>
              ))}
            </div>
                ...
                <div ref={innerRef} style={{ transform: `scale(${scale})`, transformOrigin: 'left top' }} className="select-none">
                  {layers.map((row, li) => (
                    <motion.div
                      ...
                      style={{
                        zIndex: 20 - li,
                        transform: `translateY(${li * 2}px) scale(${1 - li * 0.02})`,
                      ...
                      animate={pileSeen ? { opacity: 1, y: 0 } : {}}
                      transition={{ duration: 0.35, ease: EASE }}
                    >
                      {row.map((t, ci) => {
                        const bg = rgba(t.color, 0.3)
                        const bd = rgba(t.color, 0.5)
                        ...
                        return (
                          ...
                          >
                            ...
                            >
                              {t.icon ? (
                                <i className={t.icon} style={{ fontSize: 18, lineHeight: 0, color: t.mono ? t.iconColor || '#fff' : undefined }} />
                              ) : t.emoji ? (
                                <span style={{ fontSize: 16, lineHeight: 0 }}>{t.emoji}</span>
                              ) : t.logo ? (
                                <img src={t.logo} alt={t.name} style={{ width: 18, height: 18 }} />
                              ) : null}
                            </span>
                            {t.name}
                          </motion.span>
      ...
      <section id="clientes" className="section">
        <div className="container-max">
          ...
          {/* Estilos locales para el logo cloud */}
          <style>{`
  .logo-grid{
    display:grid;
  ...
  /* El card NO crece, solo el contenido */
  .logo-card{
  ...
  /* LOGO más grande dentro del mismo alto de card */
  .logo-img{
    max-height: 48px;
  ...
  /* Fallback (iniciales) acorde al nuevo tamaño */
  .logo-fallback{
        ...
        <div className="container-max grid md:grid-cols-2 gap-6">
          ...
          <div className="card p-6">
            <form className="grid grid-cols-1 gap-3">
              <input placeholder="Nombre" className="border border-[var(--line)] rounded-lg px-3 py-2" />
              <input type="email" placeholder="Email" className="border border-[var(--line)] rounded-lg px-3 py-2" />
              <textarea placeholder="Mensaje" rows={5} className="border border-[var(--line)] rounded-lg px-3 py-2" />
